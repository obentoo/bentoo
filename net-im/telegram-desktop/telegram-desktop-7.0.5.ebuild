...
"
# NOTE: since 7.0.0 upstream loads the xcb libraries with dlopen() instead of
#       linking against them (base_linux_xcb_library.{cpp,h} redeclares every
#       type it needs), so these are runtime-only dependencies.  Without them
#       installed the X11 integration simply stays inactive.
RDEPEND="${CDEPEND}
...
PATCHES=(
	"${FILESDIR}"/tdesktop-7.0.3-qt6-no-wayland.patch
...
src_prepare() {
	...
	# NOTE: cmark_gfm is excluded above because since 7.0.0 it locates the
	#       upstream cmake files with find_library() rather than
	#       find_package().  Gentoo's app-text/cmark-gfm ships no such files,
	#       so marking them REQUIRED would break configure before the
	#       pkg-config fallback added by our patch gets a chance to run.
...
pkg_postinst() {
	...
	if ! use X && ! use screencast; then
		...
		ewarn "the 'X' flag no longer affects the build itself: since 7.0.0 the xcb"
		ewarn "libraries are loaded with dlopen(), so it only controls whether they"
		ewarn "get pulled in as runtime dependencies."
		ewarn
	fi
	optfeature_header
