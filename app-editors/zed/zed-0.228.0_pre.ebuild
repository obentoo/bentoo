...
# To regenerate CRATES and LICENSE+=, run:
#   git clone https://github.com/zed-industries/zed --branch v0.228.0-pre
#   cd zed
#   pycargoebuild -i /path/to/this.ebuild .

inherit cargo desktop xdg

DESCRIPTION="The fast, collaborative code editor from the creators of Atom and Tree-sitter"
...
QA_FLAGS_IGNORED="usr/libexec/zed-editor"
...
src_configure() {
	...
	use wayland && myfeatures+=( wayland )
	use X && myfeatures+=( x11 )
...
src_install() {
	# Install zed-editor to libexec
	local target_dir="$(cargo_target_dir)"

	exeinto /usr/libexec
	doexe "${target_dir}/zed-editor"

	# Install CLI to /usr/bin/zed (cli expects zed-editor at ../../libexec/zed-editor)
	exeinto /usr/bin
	newexe "${target_dir}/cli" zed

	# Desktop integration
	insinto /usr/share/icons/hicolor/512x512/apps
	doins crates/zed/resources/app-icon.png

	make_desktop_entry "/usr/bin/zed %U" \
		"Zed" zed \
