...
PATCHES=(
	...
	# Trimmed for 9.14.1: only the llvm-passes hunk still applies, the
	# configure.ac LlvmMaxVersion hunk is already merged upstream.
	"${FILESDIR}/${PN}-9.14.1-llvm-passes-module.patch"
	...
	"${FILESDIR}/${PN}-9.4.5-musl-target.patch"

	# Fix QA Notice: Found the following implicit function declarations in configure logs
	"${FILESDIR}/${PN}-9.10.1-fix-configure-implicit-function.patch"

	# Support building on riscv/musl
	"${FILESDIR}/${PN}-9.10.1-llvm-targets-riscv64-unknown-linux-musl.patch"
)
