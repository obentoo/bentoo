...
if [[ ${PV} == *9999* ]]; then
	...
	RUST_TOOLCHAIN_BASEURL=https://dev-static.rust-lang.org/dist/${BETA_SNAPSHOT}/
	...
	SRC_URI="$(rust_all_arch_uris "rust-${PV}")
		rust-src? ( https://dev-static.rust-lang.org/dist/2026-01-20/rust-src-${PV}.tar.xz )
...
src_unpack() {
	if [[ ${PV} == *9999* ]]; then
		...
		curl -Ls dev-static.rust-lang.org/dist/channel-rust-nightly.toml > "${WORKDIR}/channel-rust-nightly.toml" ||
