...
if [[ ${PV} == *9999* ]]; then
	...
	SRC_URI="$(rust_all_arch_uris "rust-${PV}")
		rust-src? ( ${RUST_TOOLCHAIN_BASEURL%/}/2026-07-09/rust-src-${PV}.tar.xz )
	"
...
# NOTE(bentoo): projg2/rust-bootstrap has not published a 1.97.0 release yet, so
# these Gentoo-built bootstrap tarballs 404 (sparc64/mips*/riscv64-musl/ppc64-musl).
# Kept commented out until upstream builds them; uncomment then and re-run manifest.
