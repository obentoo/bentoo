...
# ::gentoo's 0.7.22 ships node_modules as a second, hand-rolled tarball hosted on
# dev.gentoo.org, generated by running `npm install` in the live ebuild. That is
# not reproducible from SRC_URI alone and nobody produced one for 0.8.x.
#
# It is also no longer necessary. 0.8.6 declares exactly one runtime dependency
# (terser) and no devDependencies, so `npm install --omit=optional` resolves to
# the eleven packages below -- measured, not guessed. Listing them as ordinary
# distfiles keeps the whole build fetchable, mirrorable and hash-verified by
# Portage, with no npm run at build time and no self-hosted blob.
#
# The optional dependency (v8-profiler) is deliberately absent: it drags in
# node-pre-gyp, request and ~130 further packages, and rapydscript runs without
# it. That is the same choice ::gentoo makes with --omit=optional.
#
