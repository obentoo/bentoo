...
#
# rust-patches 1.93.0 tag does not exist yet, reuse 1.92.0_p1 patchset.
...
# https://github.com/rust-lang/llvm-project/blob/rustc-1.93.0/llvm/CMakeLists.txt
...
src_prepare() {
	# Remove patch incompatible with 1.93.0 source; apply equivalent via sed below
	rm -f "${WORKDIR}/rust-patches-${RUST_PATCH_VER}/1.92.0-compiler-musl-dynamic-linking.patch" || die
	...
	# Apply musl dynamic linking fix (equivalent to 1.92.0-compiler-musl-dynamic-linking.patch)
	# Removes crt_static_default = true from musl target specs so dynamic linking is default
	# Upstream has not resolved compiler-team#422 as of 1.93.0
	local musl_targets=(
	...
	for target in "${musl_targets[@]}"; do
		local f="compiler/rustc_target/src/spec/targets/${target}.rs"
		[[ -f "${f}" ]] || continue
		sed -i \
			-e '/FIXME(compiler-team#422)/d' \
			-e '/crt_static_default = true/d' \
			"${f}" || die "Failed to patch ${f}"
	done
	einfo "Applied musl dynamic linking fix via sed for 1.93.0 compatibility"
}
