...
# v260: kernel baseline raised to 5.10
MINKV="5.10"
...
COMMON_DEPEND="
	...
	elfutils? ( >=dev-libs/elfutils-0.177:0= )
	fido2? (
		dev-libs/libfido2:0=
	)
	...
	seccomp? ( >=sys-libs/libseccomp-2.4.0:0= )
...
pkg_pretend() {
	...
	if [[ ${MERGE_TYPE} != buildonly ]]; then
		...
		# v260: KCMP always required (kernel >= 5.10 baseline)
		CONFIG_CHECK+=" ~KCMP"
...
multilib_src_configure() {
	local myconf=(
		...
		-Dsplit-bin=false
		# v260: SysV support fully removed upstream; options deprecated but still accepted
		-Dsysvinit-path=
		-Dsysvrcnd-path=
		# v260: enable compat runlevel targets (runlevel[0-6].target)
