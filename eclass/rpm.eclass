...
rpm_unpack() {
	...
	for a in "$@" ; do
		...
		if [[ ${a} = *.src.rpm ]]; then
			...
			# bentoo override: legacy RPM v3 (gzip) has no "PayloadIs" tag;
			# empty match is valid and handled by the case below (=> none).
			payload=$($(tc-getSTRINGS) "${a}" | grep -o 'PayloadIs[a-zA-Z]*' || true)
		fi
