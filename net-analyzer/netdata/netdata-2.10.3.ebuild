...
# TODO (upstream breaking changes since 2.5.3 - may need follow-up):
# - 2.10.0: C17/C++17 minimum required; Protobuf and Abseil versions bumped
# - Rust components introduced via Corrosion (FetchContent fork at
#   github.com/netdata/corrosion stable/v0.5). Disabled here by forcing
#   ENABLE_PLUGIN_OTEL, ENABLE_PLUGIN_OTEL_SIGNAL_VIEWER, and
#   ENABLE_NETDATA_JOURNAL_FILE_READER all to OFF. Future Rust enablement
#   would need a USE flag, vendored Corrosion in SRC_URI, and crate vendoring.
# - Journal viewer plugin is now a hard dependency on native packages
# - New CMake flags: ENABLE_ND_MCP added; LZ4 handling reworked;
#   IBM MQ FetchContent fix applied upstream
# - New optional plugins: NetFlow/IPFIX/sFlow, SNMP topology, Linux audit
#   subsystem -- these may need new IUSE flags and CMake -D options
...
src_configure() {
	local mycmakeargs=(
		...
		-DENABLE_PLUGIN_OTEL_SIGNAL_VIEWER=OFF # avoid Corrosion (Rust) FetchContent
		-DENABLE_NETDATA_JOURNAL_FILE_READER=OFF # avoid Corrosion (Rust) FetchContent
