...
# on a nodejs that is still ~arch there).  This overlay keeps the package at
# ~arch, so nothing forces the floor down, and net-libs/nodejs here is slotted
# by major -- the only candidates are 24.19.0-r2 and 26.7.0-r2, both above
...
# inside the gate.
# Re-check on every bump; drop if upstream Gentoo adds it.
#
# The IUSE axis follows from this line, not from an edit to IUSE: llvm-r1
# generates llvm_slot_<n> from LLVM_COMPAT, so the overlay exposes
# llvm_slot_22 where ::gentoo exposes llvm_slot_21.
# BENTOO-DIVERGENCE: IUSE - llvm_slot_22 vs llvm_slot_21, generated by the
# LLVM_COMPAT=( 22 ) above; see that comment for why 22 is inside the gate.
# BENTOO-DIVERGENCE: REQUIRED_USE - "^^ ( llvm_slot_22 )" vs "^^ ( llvm_slot_21 )",
# the same generated pair as the IUSE tag above and from the same single line.
LLVM_COMPAT=( 22 )
...
SLOT="stable"
# bentoo: the copied base is a stabilised ebuild; this overlay has not run
# .173 anywhere, so the stable keywords do not travel with the copy.
...
chromium_configure() {
	...
	if use !bundled-toolchain; then
		...
		# use_lld=true: the two are independent - use_lld picks the linker,
		# use_mold still governs linker_path. Left true, linker_path resolves to
		...
		# Scope, measured against build/config/compiler/compiler.gni at the
		# 151.0.7922.173 tag: the declare_args default is
		#   build_with_chromium && is_clang && !using_sanitizer &&
		#   !is_official_build && !is_cronet_build && linux|android && x64|arm64|x86|arm
		# so this bites USE=-official only - with the default +official,
		# is_official_build=true already makes it resolve false. That is exactly
		...
		# gn gen four times over one WORKDIR and varying a single arg:
		# with is_official_build=false and this line removed, toolchain.ninja
		# names buildtools/third_party/mold/cipd/mold 19 times; with the line back
		# it names it 0 times. Count it in toolchain.ninja - build.ninja reads 0
		# in every case and will tell you the bug is gone when it is not.
		# ::gentoo is uncovered as of .169-r1: .137 papered over it by symlinking
		# /usr/bin/mold into buildtools/third_party/mold/cipd/mold (plus a
		# >=sys-devel/mold-2.41.0 dep), and .169-r1 deleted both without adding
		# use_mold=false, so the CIPD path is reachable there again.
